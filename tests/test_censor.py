...
from assignment1.main import censor_text
...
class TestCensorText(unittest.TestCase):
    ...
    def test_censor_dates(self):
        ...
        args = argparse.Namespace(names=False, dates=True, phones=False, address=False)
        CENSOR_CHAR = '█'
    ...
    def test_censor_all(self):
        ...
        args = argparse.Namespace(names=True, dates=True, phones=True, address=True)
        CENSOR_CHAR = '█'

        censored_text = censor_text(text, entity_types, args, self.nlp, CENSOR_CHAR, self.ner_pipeline)
        # Assert that names, dates, phones, and addresses are censored
        self.assertEqual(censored_text, "████████ lives in ███ ████ ██. He can be reached at +1-██████████. He was born on ██████████ in ████ ██, ██████. ██████████ was his neighbour in ████████, ████████.")
